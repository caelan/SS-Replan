(define (domain nvidia-tamp)
  ...
  (:action detect ; TODO: or in carry conf
    ...
    :effect (and (Localized ?o1) (On ?o1 ?o0)
                 ...
                 (forall (?wp3) (when (and (WorldPose ?o1 ?wp3) (AtWorldPose ?o1 ?wp3))
  ...
  (:derived (UnsafeRelPose ?o1 ?rp1 ?s) (and (RelPose ?o1 ?rp1 ?s) (Entity ?o1)
    (exists (?o2 ?rp2) (and (RelPose ?o2 ?rp2 ?s) (Entity ?o2) (not (= ?o1 ?o2)) ; (Sample ?rp2)
                            (not (CFreeRelPoseRelPose ?o1 ?rp1 ?o2 ?rp2 ?s))
                            (AtRelPose ?o2 ?rp2 ?s)))
  ))
  (:derived (UnsafeApproach ?o1 ?wp1 ?g) (and (WorldPose ?o1 ?wp1) (Grasp ?o1 ?g)
    (exists (?o2 ?wp2) (and (WorldPose ?o2 ?wp2) (Obstacle ?o2) (not (= ?o1 ?o2)) ; (Sample ?wp2)
  ...
  (:derived (UnsafeATraj ?at) (and (ATraj ?at)
    (exists (?o2 ?wp2) (and (WorldPose ?o2 ?wp2) (Obstacle ?o2) ; (Sample ?wp2)
                            (not (CFreeTrajPose ?at ?o2 ?wp2))
                            (AtWorldPose ?o2 ?wp2)))
  ))
  (:derived (OccludedRay ?r) (and (Ray ?r) (or
    (exists (?o ?wp) (and (WorldPose ?o ?wp) (Obstacle ?o) ; (Sample ?wp)
